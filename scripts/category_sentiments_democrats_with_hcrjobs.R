work_dir<-"C:/Users/uia91182/Desktop/aba project/datasets/democrats datasets"
getwd()
setwd(work_dir)
...
data <- fread("democrats_with_hcrjobs.csv", sep=",", header=T, strip.white = T, na.strings = c("NA","NaN","","?") , stringsAsFactors = FALSE)
#View(data)
summary(data)
...
text_df <- data_frame(line = 1:866, text = data$x)
...
table(category_sentiments)
category_sentiments
write.csv(category_sentiments, "category_sentiments_democrats_with_hcrjobs.csv")
...
table(category_sentiments)
write.csv(category_sentiments, "category_sentiments_democrats_with_hcrjobs_Syuzhet.csv")
category_sentiments
