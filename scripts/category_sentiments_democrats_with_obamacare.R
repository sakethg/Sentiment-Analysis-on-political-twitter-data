...
data <- fread("democrats_with_obamacare.csv", sep=",", header=T, strip.white = T, na.strings = c("NA","NaN","","?") , stringsAsFactors = FALSE)
#View(data)
summary(data)
...
text_df <- data_frame(line = 1:1474, text = data$x)
...
table(category_sentiments)
category_sentiments
write.csv(category_sentiments, "category_sentiments_democrats_with_obamacare.csv")
...
table(category_sentiments)
write.csv(category_sentiments, "category_sentimentsdemocrats_with_obamacaresyuzhet.csv")
category_sentiments
