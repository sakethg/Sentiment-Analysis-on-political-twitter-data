...
data <- fread("republicans_with_hcrjobs.csv", sep=",", header=T, strip.white = T, na.strings = c("NA","NaN","","?") , stringsAsFactors = FALSE)
#View(data)
summary(data)
...
table(category_sentiments)
category_sentiments
write.csv(category_sentiments, "category_sentiments_rep_hcrjobs.csv")
...
table(category_sentiments)
write.csv(category_sentiments, "category_sentiments_rep_hcrjobs_Syuzhet.csv")
category_sentiments
