...
data <- fread("republicans_with_healthcare.csv", sep=",", header=T, strip.white = T, na.strings = c("NA","NaN","","?") , stringsAsFactors = FALSE)
#View(data)
summary(data)
...
text_df <- data_frame(line = 1:1312, text = data$x)
...
table(category_sentiments)
category_sentiments
write.csv(category_sentiments, "category_sentiments_republicans_with_healthcare.csv")
...
table(category_sentiments)
write.csv(category_sentiments, "category_sentimentsrepublicans_with_healthcareSyuzhet.csv")
category_sentiments
