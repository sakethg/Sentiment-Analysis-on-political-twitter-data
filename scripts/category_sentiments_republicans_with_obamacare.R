...
data <- fread("republicans_with_obamacare.csv", sep=",", header=T, strip.white = T, na.strings = c("NA","NaN","","?") , stringsAsFactors = FALSE)
#View(data)
summary(data)
...
text_df <- data_frame(line = 1:4475, text = data$x)
...
table(category_sentiments)
category_sentiments
write.csv(category_sentiments, "category_sentiments_republicans_with_obamacare.csv")
...
table(category_sentiments)
write.csv(category_sentiments, "category_sentimentsrepublicans_with_obamacareSyuzhet.csv")
category_sentiments
