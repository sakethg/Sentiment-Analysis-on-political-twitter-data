...
data <- fread("democrats_with_energyhousecommercecommittee.csv", sep=",", header=T, strip.white = T, na.strings = c("NA","NaN","","?") , stringsAsFactors = FALSE)
View(data)
summary(data)
...
text_df <- data_frame(line = 1:1215, text = data$x)
...
mydata <-read.csv("democrats_with_energyhousecommercecommittee.csv",stringsAsFactors = FALSE)
