...
mydata <-read.csv("democrats_tweets (1).csv",stringsAsFactors = FALSE)
...
corpus <- tm_map(corpus, removeWords, c("amp","will","ca10", "live", "rt", "tcot", "bit.ly","changes", "changed", "tinyurl.com", "http","new","just","today"))
...
library(topicmodels)
library(slam)
data <- mydata[1:19321,] # We perform LDA on the tweets 1 through 1000 in the data.
