...
data <- fread("republicans_with_energyhousecommercecommittee.csv", sep=",", header=T, strip.white = T, na.strings = c("NA","NaN","","?") , stringsAsFactors = FALSE)
#View(data)
summary(data)
...
text_df <- data_frame(line = 1:685, text = data$x)

head(text_df)

library(tm)

mydata <-read.csv("republicans_with_energyhousecommercecommittee.csv",stringsAsFactors = FALSE)
